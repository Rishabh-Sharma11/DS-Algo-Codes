...
public class BST_ReplaceWithSum {
	private class Node {
		int data;
		Node left;
		Node right;
	...
	private int replaceWLS(Node node) {
		if (node.right == null && node.left == null) {
			return node.data;
		}
		int lr=replaceWLS(node.left);
		int rr = replaceWLS(node.right);
		int temp=node.data;
		node.data = rr;
		return temp+lr+rr;
	...
	public static void main(String[] args) {
		BST_ReplaceWithSum b1 = new BST_ReplaceWithSum();
